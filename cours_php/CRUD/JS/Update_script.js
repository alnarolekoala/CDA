...
let discTitle = document.getElementById('disc_title');
...
function check (){
    ...
    //champ artist
    //reset du champ
    let valeurErreur6 = document.getElementById("MsgErreur6");//récupération de l'élément html
    valeurErreur6.innerHTML = "";// on ajoute du texte
    // si le champ est vide
    if(artist.value == "")
    {
        
        valeurErreur6 = document.getElementById("MsgErreur6");//récupération de l'élément html
        valeurErreur6.innerHTML = "* Remplir le champ 'Artist' svp"// on ajoute du texte
        return false;
    }
    // si ca ne correspond pas a la regex 
    else if(!(artist.value.match(artistRegex)))
    {
        valeurErreur6 = document.getElementById("MsgErreur6");//récupération de l'élément html
        valeurErreur6.innerHTML = "* Caractère(s) non autorisé(s) sur le champ 'Artist'"// on ajoute du texte
        return false;
    }
...
document.getElementById("checkForm").addEventListener("submit", check);
